---
title: Download RbLighting
description: Download RbLighting here.
---

You can find the latest version of RbLighting here:

[Download RbLighting v1.3.31](/downloads/RbLighting_1.3.32.msi)


## Changelog

### Release v1.3.32

* Bug fix: Fixed access denied message after last update.

### Release v1.3.31

* Correctly set service permissions. Fixes bug when installing on non-English Windows OS'.

### Release v1.3.28

* Fixed bug with crossfader.

### Release v1.3.27

* Fixed update notification.

### Release v1.3.26

* v1.3 release!
* UI Overhaul
* Added support for Rekordbox 7.2.4
* Added support for Rekordbox 7.2.7
* Added support for Rekordbox 7.2.8
* Added support for Rekordbox 6.8.6
* Fixed SoundSwitch integration sending wrong values for Deck 2 time.

### Release v1.2.7

* Backport: Added support for Rekordbox 7.2.8

---

### Release v1.2.6

* Backport: Added support for Rekordbox 7.2.7

---

### Release v1.2.5

* Backport: Added support for Rekordbox 7.2.4

---

### Release v1.2.4

* Improved connection UI and handling
* Backend improvements
* Added update check

---

### Release v1.1.1

* Bug fix: Fixed Device Library loading using Deck1 analysis path
* Bug fix: Fixed BPM fluctuations on RB 7.1.5

---

### Release v1.1.0

* Added support for Rekordbox 7.2.5 and 7.2.6.
* Added support for songs loaded from USB (using Device Library Plus) and streaming tracks.

